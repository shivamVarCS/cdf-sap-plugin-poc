...
public class StorageWatcher {
  ...
  public StorageWatcher() {
    ...
    bucket = storage.get("cdf-sap-dependent-files");
  }

  public boolean pollFiles(Offset offset) {
    String lrt = offset.get().get("lastReadTime");
    if(lrt != null){
      lastReadTime = Long.parseLong(lrt);
    }
    LOG.info("Last Read Time: {}", lastReadTime);
    boolean flag = false;
    ...
    while (iterator.hasNext()) {
      ...
      if (blobx.getCreateTime() > lastReadTime) {
        LOG.info("Id: " + blobx.getBlobId());
        LOG.info("Name: " + blobx.getName());
  ...
  public Offset getNewOffset(){
    ...
    offset.put("lastReadTime", Long.toString(lastReadTime));
